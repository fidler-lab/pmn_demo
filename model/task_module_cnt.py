...
'''
Counting algorithm implementation modified from https://github.com/Cyanogenoid/vqa-counting/blob/master/vqa-v2/counting.py
'''

class TaskModuleCnt(nn.Module):
    """
    ...
    [1]: Yan Zhang, Jonathon Hare, Adam Prgel-Bennett: Learning to Count Objects in Natural Images for Visual Question Answering.
    https://openreview.net/forum?id=B12Js_yRb
    """
    def __init__(self, task_v_size, map_dim, cnt_num_class, X_loc_dim, att_dim, num_glimpse, gnn_steps=0, already_sigmoided=True, use_rel=False):
    #def __init__(self, task_v_size, map_dim, objects, num_glimpse, already_sigmoided=True):
        super(TaskModuleCnt, self).__init__()
        self.name = 'TaskModuleCnt'    
        self.objects = cnt_num_class
        ...
        self.cnt_output = nn.Sequential(
            nn.Linear(self.objects+1, self.objects),
        )

    def forward(self, X_loc=None, q_vec=None, boxes=None, attention=None, v_cnt=None, mode='', graphs=[]):
        ...
        if mode == 'cnt':
            ...
            #output = self.cnt_output(input)
            #(14L, 128L, 300L)
            output = self.cnt_output(v_cnt)
            return output 
...
class PiecewiseLin(nn.Module):
    def __init__(self, n):
        super(PiecewiseLin, self).__init__()
        #super().__init__()
        self.n = n
